...
import { CSSModule, ProviderContext, ProviderProps } from "./types"
import React, { useContext, useEffect, useRef } from "react"
import styles from "@watheia/theme.styles.page/vars.css"
import typographyStyles from "@watheia/theme.styles.typography/index.css"
...
function Provider(props: ProviderProps, ref: DOMRef<HTMLDivElement>) {
  ...
  if (
    ...
    Object.keys(styleProps.style as CSSModule).length > 0
  ) {
    contents = (
      <ProviderWrapper
        {...props}
...
const ProviderWrapper = React.forwardRef(function ProviderWrapper(
  ...
  const themeKey = Object.keys(theme[colorScheme] as CSSModule)[0]
  const scaleKey = Object.keys(theme[scale] as CSSModule)[0]

  const className = clsx(
    styleProps.className,
    styles["spectrum"],
    typographyStyles["spectrum"],
    (theme[colorScheme] as CSSModule)[themeKey],
    (theme[scale] as CSSModule)[scaleKey],
